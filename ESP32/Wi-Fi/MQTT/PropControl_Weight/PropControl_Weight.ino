...
void setup(){
  ...
  scale.begin(17, 16); // Data, Clock 
  // Optionally set scale calibration factor (adjust based on your calibration)
  scale.set_scale(-384500.f); // Replace with your calibration factor
  ...
  // See https://github.com/espressif/esp-idf/blob/master/components/esp_wifi/include/esp_wifi_types_generic.h
  // for meaning of reason codes
...
void sendUpdate() {
  ...
  char formattedWeight[10]; // Enough space for "xxx.xx\0"
  snprintf(formattedWeight, sizeof(formattedWeight), "%.2fkg", weight);
  jsonDoc["input"] = formattedWeight;
