...
enum : byte { LAN_DOWN_UDP_DOWN, LAN_STARTING_UDP_DOWN, LAN_UP_UDP_DOWN, LAN_UP_UDP_UP } connectionState;
  ...
  switch (networkState) {
    ...
    case LAN_STARTING_UDP_DOWN:
      ...
      if (millis() - timeStamp >= 30000) {
        ...
        Serial.println("Failed to start network connection. Restarting...");
        // And reset the state machine to its initial state (restart)
        networkState = LAN_DOWN_UDP_DOWN;
      }
